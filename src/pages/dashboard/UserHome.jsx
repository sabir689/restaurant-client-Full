...
import { FaWallet, FaShopify, FaPhoneAlt, FaShoppingCart, FaStar, FaCalendarAlt, FaCreditCard, FaArrowRight } from 'react-icons/fa';
    ...
    return (
        <div className="w-full min-h-screen  p-4 md:p-10 text-slate-800 ">
            ...
            <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <h2 className="text-5xl font-black font-cinzel text-slate-900 mt-2">
                        Welcome, <span className="text-[#B58130] italic">{user?.displayName?.split(' ')[0] || 'Guest'}</span>
                    </h2>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                {[
                    { label: 'Menu', val: stats.menuItems || 205, icon: <FaWallet />, grad: 'from-[#BB34F5] to-[#8a2be2]', shadow: 'shadow-purple-200' },
                    { label: 'Shop', val: stats.shopCount || 103, icon: <FaShopify />, grad: 'from-[#D3A256] to-[#b58130]', shadow: 'shadow-orange-200' },
                    { label: 'Contact', val: stats.contactCount || '03', icon: <FaPhoneAlt />, grad: 'from-[#FE4880] to-[#e91e63]', shadow: 'shadow-pink-200' }
                ].map((card, i) => (
            ...
            {/* Unique Profile Experience Section */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
                
                {/* Profile Card (4/12 width) */}
                <div className="lg:col-span-5 bg-white rounded-[3rem] p-10 shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col items-center text-center relative overflow-hidden">
                    ...
                    <div className="relative mt-4">
                        <div className="w-44 h-44 rounded-[2.5rem] border-8 border-white shadow-2xl overflow-hidden transform rotate-3 hover:rotate-0 transition-transform duration-500">
                            {user?.photoURL ? (
                    ...
                    <button className="px-8 py-3 bg-slate-900 text-white rounded-2xl font-bold hover:bg-[#B58130] transition-colors flex items-center gap-2 group">
                        Edit Profile <FaArrowRight className="group-hover:translate-x-1 transition-transform" />
                    </button>
                </div>

                {/* Activities Card (7/12 width) */}
                <div className="lg:col-span-7 bg-[#1a1a1a] rounded-[3rem] p-12 text-white shadow-2xl relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {[
                            { label: 'Orders', val: stats.orders || 6, color: 'text-blue-400', bg: 'bg-blue-400/10', icon: <FaShoppingCart /> },
                            { label: 'Reviews', val: stats.reviews || 2, color: 'text-emerald-400', bg: 'bg-emerald-400/10', icon: <FaStar /> },
                            { label: 'Bookings', val: stats.bookings || 1, color: 'text-amber-400', bg: 'bg-amber-400/10', icon: <FaCalendarAlt /> },
                            { label: 'Payments', val: stats.payments || 3, color: 'text-rose-400', bg: 'bg-rose-400/10', icon: <FaCreditCard /> }
                        ].map((act, i) => (
